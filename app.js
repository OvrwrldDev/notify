...
const defaultConfig = {
    ...
    VariantDefinitions: {
        success: { classes: "success", icon: "check_circle", label: "Success" },
        primary: { classes: "primary", icon: "notifications", label: "Notice" },
        warning: { classes: "warning", icon: "warning", label: "Warning" },
        error: { classes: "error", icon: "error", label: "Error" },
        police: { classes: "police", icon: "local_police", label: "Police" },
        ambulance: { classes: "ambulance", icon: "fas fa-ambulance", label: "EMS" },
    },
};
...
const getContainer = () => {
    ...
    // Position is forced to top-right for this design regardless of what
    // getNotifyConfig returns, since the backend config was overriding it.
    const pos = "top-right";
    ...
    Object.assign(container.style, {
        ...
        ...(POSITION_MAP[pos] ?? POSITION_MAP["top-right"]),
    });
...
const showNotif = async ({ data }) => {
    ...
    item.innerHTML = `
        <div class="notify-icon-chip">${iconHtml}</div>
        <div class="notify-content">
            <div class="notify-label">${style.label ?? type ?? "primary"}</div>
